...
for (var i = 0; i < msgs.length; i++) {
    ...
    if (msgs[i].hasOwnProperty("port") && msgs[i].hasOwnProperty("payload_raw")) {
        ...
        if (msgs[i].hasOwnProperty("uplink_message")) {
			...
			if (uplink_message.hasOwnProperty("locations")){
				var locations = uplink_message.locations;
				if (locations.hasOwnProperty("user")) {
					var user = locations.user;
					if(user.hasOwnProperty("latitude") && user.hasOwnProperty("longitude")) {
						lat = user.latitude;
						lng = user.longitude;
					}
				}
